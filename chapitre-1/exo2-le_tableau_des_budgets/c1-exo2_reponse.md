# Exercice — Le Tableau des Budgets

## Enoncé
Reprenez le tableau des cinq étapes du chapitre. Pour chacune, cherchez une source, un article ou une documentation de constructeur, qui donne une valeur mesurée. Rendez le tableau avec vos sources.

## Données relevées :
 - **Mesure du mouvement** : 4 millisecondes (1 milliseconde pour le *Meta Quest 3*)
 - **Transmission de la mesure** : /
 - **Prise de décision et dessin** : 8 - 15 millisecondes
 - **Assemblage pas le compositeur** : /
 - **Affichage par l'écran** : -1 milliseconde pour les écrans OLED et de 3 à 7 millisecondes pour les LCD selon l'utilisation ou non de la technologie overdrive

## Sources : 
https://gdepark.com/fr/7-key-factors-causing-motion-to-photon-latency-in-9d-vr-systems/